...
static inline void send_event(debounce_t db, int ev) {
	ESP_LOGI(TAG, "Sending event %d", ev);
	esp_event_post(BUTTON_EVENT, ev, &db.pin, sizeof(db.pin), 100/portTICK_PERIOD_MS);
}
	...
	while (1) {
		for (int idx=0; idx<pin_count; idx++) {
			...
			if (debounce[idx].down_time && (millis() - debounce[idx].down_time > LONG_PRESS_DURATION)) {
				...
				send_event(debounce[idx], EVT_BUTTON_UP);
